...
/**
 * Common failure scenarios for easy testing.
 */
export const FAILURE_SCENARIOS = {
	dirty_workspace: {
		git: {workspace_clean: false, has_changes: true},
	},
	wrong_branch: {
		git: {current_branch: 'feature-branch'},
	},
	npm_not_authenticated: {
		npm: {authenticated: false},
	},
	npm_registry_down: {
		npm: {registry_available: false},
	},
	build_failure: {
		build: {build_fails: true, build_error_message: 'TypeScript compilation failed'},
	},
	preflight_failure: {
		preflight: {
			fails: true,
			errors: ['Workspace has uncommitted changes', 'Not on main branch'],
		},
	},
	install_failure: {
		npm: {install_fails: true},
	},
	checkout_failure: {
		git: {checkout_fails: true},
	},
	pull_failure: {
		git: {pull_fails: true},
	},
	npm_timeout: {
		npm: {wait_timeout: true},
	},
} as const;
