...
for (const fixture of FIXTURES) {
	describe(`${fixture.name} fixture`, () => {
		...
		const has_errors = fixture.expected_outcomes.errors && fixture.expected_outcomes.errors.length > 0;

		describe('gitops_analyze', () => {
			test('produces expected publishing order', async () => {
				if (has_errors) {
					...
					const result = await run_gitops_command_json('gitops_analyze', [], undefined, config_path);
				...
				assert_publishing_order(result.publishing_order, fixture.expected_outcomes.publishing_order);
			}, COMMAND_TIMEOUT);
		});
		...
		describe('gitops_plan', () => {
			test('predicts correct version changes', async () => {
				...
				if (fixture.expected_outcomes.version_changes.length > 0) {
					assert_version_changes(result.version_changes, fixture.expected_outcomes.version_changes);
				} else {
			...
			test('tracks breaking cascades', async () => {
				...
				for (const [pkg, expected_affected] of Object.entries(
					...
					assert.ok(
						result.breaking_cascades[pkg],
						`Should have breaking cascade for ${pkg}`,
					);
			...
			test('reports errors', async () => {
				if (!fixture.expected_outcomes.errors || fixture.expected_outcomes.errors.length === 0) return;
			...
			test('reports info for packages with no changes', async () => {
				if (has_errors || !fixture.expected_outcomes.info || fixture.expected_outcomes.info.length === 0)
					return;
